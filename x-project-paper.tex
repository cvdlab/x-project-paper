...
\maketitle
\begin{abstract}
This paper introduces X-Project, a next-generation API-centric HTML5 Web Components based Web Application and CMS framework.
X-project is designed to allow the development of dynamic web applications and Content Management Systems.
X-Project's goal is to provide an easy way both to build REST API, using StrongLoop/Loopback framework, and to create intuitive and fancy UIs HTML5 Web Components based, using Polymer-Project.
X-Project expects three actors using the system: developers, admins and users. Developers' job is really simplified both in backend and in frontend side. Using Strongloop framework and an automatical generation of forms script, the developer concern is limited to the JSON models' creation. Moreover X-Project provides a dedicated Web Components library to make easier the UI structuring.
Admins can take advantage of an intuitive and featured panel to management website's content.
Eventually, users benefit of a plain UI thanks to the mix of Web Components used to build pages' composition.
...
\keywords{API, Content Management System, Single Page Application, Web Application, Web Components, Web Platform}
